...
{
    /// <summary>What one column can hold: the largest byte array the runtime allocates.</summary>
    public const int MaximumBytes = 0x7FFFFFC7;
    ...
    {
        // In LONG: a column past two gigabytes overflows the running length, and an overflowed
        // length compares as "there is room", which is how too much data became "Destination is
        // too short" three layers away instead of a sentence naming the column that overflowed.
        long wanted = (long)_length + bytes.Length;
        if (wanted > MaximumBytes)
        {
            throw new InvalidOperationException(
                $"one column would hold {wanted} bytes, past the {MaximumBytes} a single array can "
                + "carry -- state this as a per-row answer rather than as one column of everything.");
        }
        if (wanted > _data.Length)
        {
            Array.Resize(ref _data, (int)Math.Min(MaximumBytes, Math.Max((long)_data.Length * 2, wanted)));
        }
