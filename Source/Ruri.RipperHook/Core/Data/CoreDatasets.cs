...
public static class CoreDatasets
{
    ...
    public static void Register()
    {
        ...
        Statements.StatementDatasets.Register();
        Statements.ClipSourceSkeleton.Register();
        Statements.TextureBake.Register();
        FolderDatasets.Register();
        ShaderDatasets.Register();
        Installs.Register();
    ...
    private static ColumnTable Select(DataRequest request)
    {
        ...
        // The container path is both the identity and what loading a row needs: it
        // is a seed of the engine's own flattening (see StatementSources).
        table.Role(ColumnRole.Label, CabPathQuery.LeafField)
            .Role(ColumnRole.Key | ColumnRole.Payload, CabPathQuery.ContainerField)
            .Role(ColumnRole.Detail, CabPathQuery.FolderField)
            .Role(ColumnRole.Facet, CabPathQuery.ExtensionField)
            .Role(ColumnRole.Group, CabPathQuery.BundleField);
