...
public static class RipperBlenderBridge
{
    ...
    private sealed class PrewarmedTextureExporter : TextureAssetExporter
    {
        ...
        /// <summary>
        /// Runs the png encoder's one-time initialisation on ONE thread, before any
        /// worker can reach it.
        ///
        /// FastPng is a machine translation of a C library: its globals initialise
        /// lazily and each type initialiser registers itself into a shared dictionary
        /// that is not synchronised. Several of those initialisers running at once
        /// collide -- "An item with the same key has already been added. Key: 1" --
        /// and because it is thrown out of a static constructor the type stays poisoned
        /// for the rest of the process, so every later texture fails too. The png
        /// writer is deliberately lockless (that is what makes it the fast container to
        /// export through), so the parallel encode below is precisely the condition
        /// that triggers it: only on the first character of a session, and only when
        /// the workers happen to arrive together -- which is why it reads as random.
        ///
        /// One pixel is enough to force the whole chain through serially. Both channel
        /// counts are warmed because a bitmap with no alpha takes the other branch into
        /// the encoder. Failure is reported and not thrown: a warm-up that cannot run
        /// is not itself a reason to fail an import that might export no png at all.
        /// </summary>
        private static readonly Lazy<bool> PngEncoderReady = new(static () =>
        {
            try
            {
                using MemoryStream rgba = new();
                new AssetRipper.Export.Modules.Textures.DirectBitmap<
                    AssetRipper.TextureDecoder.Rgb.Formats.ColorRGBA<byte>, byte>(1, 1)
                    .Save(rgba, AssetRipper.Export.Configuration.ImageExportFormat.Png);
                using MemoryStream rgb = new();
                new AssetRipper.Export.Modules.Textures.DirectBitmap<
                    AssetRipper.TextureDecoder.Rgb.Formats.ColorRGB<byte>, byte>(1, 1)
                    .Save(rgb, AssetRipper.Export.Configuration.ImageExportFormat.Png);
                return true;
            }
            catch (Exception exception)
            {
                Console.Error.WriteLine(
                    $"[texture] png encoder warm-up failed ({exception.GetType().Name}: "
                    + $"{exception.Message}); parallel png export may be unstable.");
                return false;
            }
