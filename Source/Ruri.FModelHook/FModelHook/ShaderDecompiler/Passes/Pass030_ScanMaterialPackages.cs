...
internal static class Pass030_ScanMaterialPackages
{
    ...
    private static void BuildMaterialContexts(ExportPipelineState state, AbstractVfsFileProvider provider)
    {
        ...
        // A scoped run NEVER reaches the full scan. Which branch is taken below turns on whether
        // the containers happened to state any shader-map owners -- a build cooked to IoStore
        // states thousands and goes to the bridge, a build cooked to pak states none and used to
        // fall into reading every material package the install ships. That is the same
        // whole-install cost the scope exists to refuse, and it does not stop being so because
        // the build was cooked differently.
        if (output.PackageShaderMapHashes.Count == 0 && state.MaterialScope is not { Count: > 0 })
        {
    ...
    private static void BuildResourceHashBridge(ExportPipelineState state, AbstractVfsFileProvider provider)
    {
        ...
        // A scope REPLACES the install-wide set rather than adding to it. The container headers
        // name every package the build ships that owns a shader map -- tens of thousands -- and
        // reading all of them is exactly the cost a caller asking about one character is avoiding.
        HashSet<string> containerKeys;
        HashSet<string> packageSet;
        if (state.MaterialScope is { Count: > 0 } scope)
        {
            containerKeys = new HashSet<string>(scope, StringComparer.OrdinalIgnoreCase);
            packageSet = new HashSet<string>(scope, StringComparer.OrdinalIgnoreCase);
            log($"    Material bridge: scoped to {scope.Count} material package(s) the caller asked about.");
        }
        else
        {
            containerKeys = new HashSet<string>(output.PackageShaderMapHashes.Keys, StringComparer.OrdinalIgnoreCase);
            packageSet = new HashSet<string>(containerKeys, StringComparer.OrdinalIgnoreCase);
