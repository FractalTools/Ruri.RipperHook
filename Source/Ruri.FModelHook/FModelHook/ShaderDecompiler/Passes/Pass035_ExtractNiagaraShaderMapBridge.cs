...
internal static class Pass035_ExtractNiagaraShaderMapBridge
{
    public static void DoPass(ExportPipelineState state)
    {
        ...
        // A run scoped to some material packages is not about the build's effects: a character's
        // materials are never Niagara systems, and reading every one the install ships to find
        // that out costs more than the whole export it is part of.
        if (state.MaterialScope is { Count: > 0 })
        {
            state.Log("    Niagara bridge: SKIPPED — this run is scoped to stated material packages.");
