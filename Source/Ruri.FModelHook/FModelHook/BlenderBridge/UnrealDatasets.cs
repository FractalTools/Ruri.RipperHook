...
public static class UnrealDatasets
{
    ...
    /// <summary>
    /// Run the PNG encoder ONCE, on one thread, before any parallel encode -- and run it over
    /// pixels that reach the encoder's DYNAMIC path, not just its trivial one.
    ///
    /// The encoder keeps its constant tables in globals whose initializers register themselves
    /// into one shared, unsynchronised dictionary. Several of those initializers can therefore
    /// run at once when several threads first reach them together, and the dictionary comes
    /// apart -- reported as "An item with the same key has already been added", thrown out of a
    /// type initializer, which is permanent for the process: every texture after it fails too.
    ///
    /// A one-pixel warm-up does not prevent that, because a single pixel never costs enough for
    /// the encoder to choose the dynamic tables, so the very globals that crash stay untouched
    /// until the parallel loop reaches them. So the warm-up encodes something big and noisy
    /// enough to be worth compressing, which is what makes it take that path here, alone.
    /// </summary>
    private static readonly Lazy<object> Warmed = new(static () =>
    {
        const int Side = 64;
        DirectBitmap<ColorBGRA<byte>, byte> bitmap = new(Side, Side, 1);
        Span<ColorBGRA<byte>> pixels = bitmap.Pixels;
        // Wrapping multiply and narrowing casts are both the POINT of the generator, and this
        // assembly is built with arithmetic checks on -- which is why this says so outright.
        unchecked
        {
            uint noise = 0x1234_5678;
            for (int index = 0; index < pixels.Length; index++)
            {
                // Cheap deterministic noise: a run of one colour compresses with the trivial
                // path, which is the path that does NOT initialize what we came to initialize.
                noise = (noise * 1664525u) + 1013904223u;
                pixels[index] = new ColorBGRA<byte>(
                    (byte)((noise >> 24) & 0xFF), (byte)((noise >> 16) & 0xFF),
                    (byte)((noise >> 8) & 0xFF), byte.MaxValue);
            }
        }
        using MemoryStream stream = new();
        bitmap.SaveAsPng(stream);
    ...
    {
        TableBuilder table = new(ActorsId, "package", "name|Name", "kind|Kind", "parent", "native",
            "skeletal#", "static#");
        table.Role(ColumnRole.Label, "name").Role(ColumnRole.Key | ColumnRole.Payload, "package")
            .Role(ColumnRole.Group, "kind");
        UnrealFileProvider provider = UnrealProviderSession.Open(request.GameRoot);
    ...
    {
        TableBuilder table = new(WorldsId, "world", "name|Name", "partitioned", "cells#", "minX#", "minY#", "maxX#", "maxY#");
        table.Role(ColumnRole.Label, "name").Role(ColumnRole.Key | ColumnRole.Payload, "world");
        if (!request.HasMap)
        {
            throw new InvalidOperationException(
    ...
    private static ColumnTable SessionState(DataRequest request)
    {
        TableBuilder table = new(SessionId, "project", "displayName", "engine", "engineVersion", "files#", "archives#",
            ...
            "characterForwardX#", "characterForwardY#", "characterForwardZ#");
        ...
        // Stated in the engine's axes by the title, handed over in the basis every other
        // coordinate of this session crosses in -- one conversion, here, not one per host.
        System.Numerics.Vector3 characterForward = UnrealStatementSource.CharacterForward(claim);
        table.Row(
