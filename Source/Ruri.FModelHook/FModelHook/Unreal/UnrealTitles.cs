...
public sealed record UnrealTitle
{
    ...
    /// <summary>
    /// Which way a character asset's OWN space faces, in this engine's axes.
    ///
    /// An engine states where its axes point; it does not state which way a studio modelled its
    /// cast. A placed actor needs no such statement -- its component transform turns the mesh to
    /// the actor's forward -- but a character imported as a bare skeletal mesh has no component,
    /// so the only thing that can say which way that mesh faces is the build itself. The default
    /// is the engine's own forward, which is what an asset authored to be used without a turn
    /// faces; a studio that modelled its cast facing sideways states that here.
    ///
    /// Read by the session row, converted through <see cref="UnrealBasis"/> first, so a host is
    /// handed the facing in the basis it already works in and never has to know Unreal's axes.
    /// </summary>
    public System.Numerics.Vector3 CharacterForward { get; init; } = EngineForward;

    /// <summary>Where this engine's own forward points, in its own axes -- what an asset
    /// authored to be used without a turn faces, and what an install that states nothing about
    /// its cast is taken to mean.</summary>
    public static readonly System.Numerics.Vector3 EngineForward = new(1f, 0f, 0f);
