...
13. **测试循环**:🛑 **产物禁落 D 盘**(机械盘,重 IO 会把整机卡死,连 `grep -r`/`ls -R` 都会超时)——首选全程内存(UE 那条路本就直出宿主、不落盘);必须落盘时一律 `C:\Users\Zhang\Desktop\Temp\RipperHookImportOutput`(每次运行前清空,禁塞额外文件夹);开新运行前先杀残留 `Ruri.RipperHook.CLI.exe`;长运行走 `run_in_background`+`Monitor` until-loop,禁用短 sleep 串绕过死锁守卫。
