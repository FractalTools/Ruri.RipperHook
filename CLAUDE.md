...
Ruri-RipperHook 是 `RuriRipperImporter`（Blender 插件）的上游数据管线：把 Unity / UE 项目里的网格、材质、骨架、动画等资产解析、转换、导出成标准 Unity YAML（AssetRipper 原生产物形态），供该插件直接读入 Blender 重建——服务于角色建模、绑骨、MMD 风格动画等 3D 内容创作工作流。更新文档、摘要或对外描述时，围绕这个定位来写。具体某个游戏的兼容层实现（解析它的私有资源容器格式）住在独立子模块 `AssetRipperGameHook` 里——那是各游戏各自的适配细节，不属于本仓库对外描述的范围（本仓库本身是通用的解包 / 导出 / 数据处理工具链）。
