...
- **Frozen upstream is sacred.** Behaviour over AssetRipper / submodules is added by hooks/modules **inside the existing `Ruri.*` projects** only (§1) — never by editing the frozen tree, and **never by spinning up a new assembly**. The "extension point" is a hook/module registration in the core, not a new project. Don't-touch and design-for-extension are the same coin.
...
| Rule | Detail |
|---|---|
| Editable area | The **existing** `Source/Ruri.*/**` projects (Ruri.RipperHook, Ruri.AssemblyDumper, Ruri.Hook, Ruri.SourceGenerated, Ruri.ShaderDecompiler). Edit these in place. |
| **No new assemblies** | **Never create a new `.csproj` / project / assembly for a feature — not even a `Ruri.*`-named one.** Every feature lands **inside an existing `Source/Ruri.*` project** (default: `Ruri.RipperHook` core) as Ruri.Hook attribute hooks plus their supporting code. New NuGet dependencies — even heavy/native ones (e.g. a USD binding) — go on that existing csproj. If you catch yourself scaffolding a project to "isolate" a dependency, an exporter, or for "extensibility", **stop** — that instinct (generic §0.C) is wrong here; add a hook to the core instead. |
...
| **Export sees pure Unity data** | Game decryption, ACL decode, and custom container formats are made transparent by upstream **read-path** hooks — by the time any processing/export code runs, AR holds **pure, genuine Unity data** (standard source-gen types; clip curves already decoded; meshes already de-streamed). **Never** re-handle decryption / ACL / custom formats at export time. A new export format (e.g. USD) is added by **replacing or augmenting an AR export method via a hook** and consuming AR's already-clean model directly — not by a parallel service that re-derives the data. |
