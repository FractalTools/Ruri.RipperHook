...
- **humanoid 烘焙**（`HumanoidSolver/`）：`AvatarMuscleReferential`（Avatar Axes：PreQ/PostQ/Sgn/Limit + TOS 路径 + 全 95 肌肉 DOF 表：55 身体含眼颚 + 40 手指经 `Hand.HandBoneIndex` finger-major）+ `HumanoidClipBaker`（muscle 曲线按 SampleRate 采样 → `preQ*swingTwist*postQ⁻¹` 直接就是该骨骼**绝对** local rotation，不是 delta，不跟 rest 复合；hips = RootT−MotionT + RootQ 前乘 rest（已知错，见下）；MotionT/Q 烘到根节点，复合恒等于完整 RootT 轨迹）。
  ...
  - **★根运动坑（发现于 2026-07-10，未修复，与上面三轮公式坑完全独立）**：用真实 prefab 摆放骨骼验证时发现——即使公式部分完全修好、每根骨骼**独立**的 local rotation 都已核对匹配 Unity 真值，渲染出的姿势仍然是手/腿甩到头顶上方，穿模。逐层排查（rest pose 校验、hierarchy.py 世界矩阵校验、Blender bone.matrix_local 校验，全部逐项对照 Unity 数值，逐一验证为零误差吻合）后定位到：**Hips 自己的世界位置/旋转就是错的**——`body_transform()` 直接把 `RootT`/`RootQ`（Unity Animator 级根运动通道）当成"Hips 骨骼自身的 local rotation"喂给 Hips 骨骼，但实测 Unity 里 Hips 骨骼自己的 rest local rotation 是**精确单位阵**、动画中的 local 变化也很小（~7°/~0.003m），而 `RootT.y` 在 f=0 时读到 0.9928，Unity 实测 Hips 当帧 local position.y 却是 0.9006（rest 是 0.8974）——两者对不上（~0.09 米量级偏差 + 8.5° 旋转偏差）。怀疑真相：`RootT`/`RootQ` 是 Unity Animator 组件所在物体（骨架的上一级容器，`hierarchy.py` 里 `Root` 节点的 `parent`）的根运动，不该直接套在 "Hips"/`Root` 骨骼自己身上；Hips 骨骼自身可能压根没有独立 muscle delta，或者需要另一套读法。**这是与 humanoid 公式修复完全独立的另一个 bug，尚未定位到确切正确写法、尚未修复**，是当前"改完公式渲染仍然穿模"的直接原因，后续会话优先级最高。

Python (`RuriRipperImporter/humanoid_retarget.py`) 与 C# (`AvatarMuscleReferential.LocalRotation`) 两份实现已同步第三轮（最终）公式。**教训（三轮事故共同指向同一件事）：数值自洽（对称性/连续性/无 NaN）、单一测试骨骼吻合、甚至"扩大到 15 根骨骼验证通过"都不能证明 humanoid 解算全局正确——取真值的脚本本身、以及公式接入真实调用点之后的实际效果，都要跟结果本身一样被怀疑、被复核，多关节、多帧对 Unity 真值核对没有捷径。**
