...
- **humanoid 烘焙**（`HumanoidSolver/`）：`AvatarMuscleReferential`（Avatar Axes：PreQ/PostQ/Sgn/Limit/NormalizedRest + TOS 路径 + 全 95 肌肉 DOF 表：55 身体含眼颚 + 40 手指经 `Hand.HandBoneIndex` finger-major）+ `HumanoidClipBaker`（muscle 曲线按 SampleRate 采样 → `rest * normRest⁻¹*preQ*swingTwist*postQ⁻¹`；hips = RootT−MotionT + RootQ 前乘 rest；MotionT/Q 烘到根节点，复合恒等于完整 RootT 轨迹）。
  - **★公式坑第一轮（真实事故，2026-07-10）**：最初两轮实现都用 `postQ*swingTwist*postQ⁻¹`（对称共轭）——数值自检（L/R 镜像对称、无 NaN、帧间平滑）全部通过，视觉上也"像"在走路，但**用真实 Unity Editor（unityMCP `execute_code` + `AnimationMode.SampleAnimationClip` 在活的 Avatar+clip 上采样真实骨骼旋转）核对后发现整条腿关节方向全反**——SaionNanae 测试角色膝盖肉眼可见反向弯曲。真相：`preQ` 和 `postQ` 在真实 rig 上根本不是同一个东西（同一骨骼二者可差 60°~280°），公式必须是**非对称**的 `preQ * swingTwist * postQ⁻¹`。当时**误以为**还需要统一加一个 `normRest⁻¹` 前缀（因为 muscle=0 时 `preQ@postQ⁻¹` 不收敛到单位阵，而某些骨骼如 UpperLeg 这个基线离单位阵能到 ~184°）——这个"统一加"的结论**只用左腿数据验证，是本次事故的第二个坑，见下条**。

  - **★公式坑第二轮（真实事故，2026-07-10，同日）**：用户反馈"手完全错的，穿模到身体里"，扩大验证到 15 根骨骼（躯干 5 根 + 肩 2 根 + 四肢 8 根）× 11 帧 × Unity 真值。发现 `normRest⁻¹` 前缀**不是全局常量，是逐骨骼、数据驱动的开关**：Spine/Chest/UpperChest/`Left*`(四肢) 不需要（强行加上会从 <10° 误差炸到 170°+）；Neck/Head/`Left/RightShoulder`/`Right*`(四肢) 需要（不加会从 0° 炸到 15°~120°）。已验证与"需要"完全 1:1 相关的唯一信号是 `sgn[0]`（twist 轴符号）**仅对四肢**成立（`sgn[0]<0 → 需要`），躯干/肩必须单独枚举，因为它们没有独立 twist DOF、`sgn[0]` 值无意义。已排除：单纯 preQ/postQ 幅值差、preQ·normRest 点积符号、L/R 命名本身。怀疑是 Unity 内部固定的"归一化骨架"模板本身自带的骨骼级差异（同一模板服务所有 avatar），而非本 avatar 专属——**但只在一个 avatar 上验证过，未跨角色复核，移植/使用前必须用同样方法论重新核对**。同时发现 `Left Forearm Twist In-Out` 需要对 twist 角度额外取反（双侧一致，与 `sgn[0]`/L-R 无关，根因未查明，按经验值处理）。最终公式见 `humanoid_retarget.py` 的 `_NEEDS_NORM_REST_CORRECTION`/`_TWIST_SIGN_FLIP` 表 + 详细 docstring。残留已知缺口：`RightUpperArm` 即使加了修正仍平均 ~44° 偏差，穷举摆动合成顺序/符号/preQ-postQ 互换均未能进一步收敛，未查明根因，未修补。全量验证：187 个 (骨骼,帧) 样本，平均误差从原始 40°~190°+ 降到 6.78°。

  - **★根运动坑（发现于 2026-07-10，未修复）**：用真实 prefab 摆放骨骼验证时发现——即使上面两轮公式修好、每根骨骼**独立**的 local delta 都已核对匹配 Unity 真值，渲染出的姿势仍然是手/腿甩到头顶上方，穿模。逐层排查（rest pose 校验、hierarchy.py 世界矩阵校验、Blender bone.matrix_local 校验，全部逐项对照 Unity 数值，逐一验证为零误差吻合）后定位到：**Hips 自己的世界位置/旋转就是错的**——`body_transform()` 直接把 `RootT`/`RootQ`（Unity Animator 级根运动通道）当成"Hips 骨骼自身的 local delta"喂给 Hips 骨骼，但实测 Unity 里 Hips 骨骼自己的 rest local rotation 是**精确单位阵**、动画中的 local 变化也很小（~7°/~0.003m），而 `RootT.y` 在 f=0 时读到 0.9928，Unity 实测 Hips 当帧 local position.y 却是 0.9006（rest 是 0.8974）——两者对不上（~0.09 米量级偏差 + 8.5° 旋转偏差）。怀疑真相：`RootT`/`RootQ` 是 Unity Animator 组件所在物体（骨架的上一级容器，`hierarchy.py` 里 `Root` 节点的 `parent`）的根运动，不该直接套在 "Hips"/`Root` 骨骼自己身上；Hips 骨骼自身可能压根没有独立 muscle delta，或者需要另一套读法。**这是与本次 humanoid 公式修复完全独立的第三个 bug，尚未定位到确切正确写法、尚未修复**，是当前"改完公式渲染仍然穿模"的直接原因，后续会话优先级最高。

Python (`RuriRipperImporter/humanoid_retarget.py`) 与 C# (`AvatarMuscleReferential.cs`) 两份实现——**Python 已同步第二轮公式，C# 尚未跟进（1:1 义务未完成，下一步）**；此前 FRAMEWORK 记录的"1:1 移植自已验证的 humanoid_retarget.py"说法本身也是第一轮事故的一部分——那份"已验证"从未真正对过 Unity 真值。**教训（两轮重复验证到的同一件事）：数值自洽（对称性/连续性/无 NaN）、甚至单一测试骨骼对 Unity 真值吻合，都不能证明 humanoid 解算全局正确——必须多关节、多帧、逐一对 Unity 真值核对，覆盖不到的骨骼默认视为未验证。**
