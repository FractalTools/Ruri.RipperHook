...
- **humanoid 烘焙**（`HumanoidSolver/`）：`AvatarMuscleReferential`（Avatar Axes：PreQ/PostQ/Sgn/Limit + TOS 路径 + 全 95 肌肉 DOF 表：55 身体含眼颚 + 40 手指经 `Hand.HandBoneIndex` finger-major）+ `HumanoidClipBaker`（muscle 曲线按 SampleRate 采样 → `preQ*swingTwist*postQ⁻¹` 直接就是该骨骼**绝对** local rotation，不是 delta，不跟 rest 复合；hips = RootT−MotionT + RootQ 前乘 rest（已知错，见下）；MotionT/Q 烘到根节点，复合恒等于完整 RootT 轨迹）。
  ...
  - **★根运动坑（发现于 2026-07-10，已于同日修复）**：用真实 prefab 摆放骨骼验证时发现——即使公式部分完全修好、每根骨骼**独立**的 local rotation 都已核对匹配 Unity 真值，渲染出的姿势仍然是手/腿甩到头顶上方，穿模。逐层排查（rest pose 校验、hierarchy.py 世界矩阵校验、Blender bone.matrix_local 校验，全部逐项对照 Unity 数值，逐一验证为零误差吻合）后定位到：**Hips 自己的世界位置/旋转就是错的**——旧版 `body_transform()` 直接把 `RootT`/`RootQ` 当成"Hips 骨骼自身的 local rotation"喂给 Hips 骨骼，但实测 Unity 里 Hips 骨骼自己的 rest local rotation 是精确单位阵、动画中的 local 变化也很小，`RootT.y` 却读到明显更大的值——两者对不上。
    ...
    - **第二坑（同日续修）：没有 `MotionT`/`MotionQ` 曲线的 clip 位移仍然发散**：用真实 WalkLoop clip 端到端核对（非 GUI 目视,headless Blender 直接读烘焙后的 F-curve 数值）时发现,虽然 Hips 自身旋转已经正确,但 Hips 的位置(尤其沿走路方向的分量)仍随时间单调增长到 1米以上量级,和视觉上"角色在原地表演走路"或"飞出画面"的症状吻合。根因：该 clip 的 `m_KeepOriginalPositionXZ`/`m_KeepOriginalOrientation` 均为 `false`（对应 C# `IMuscleClipInfo.KeepOriginalPositionXZ`/`KeepOriginalOrientation`,Python 端 `m_AnimationClipSettings` 同名字段），即 XZ 位移与朝向(yaw)本该被"提取"记在角色根节点上,不属于 Hips；但因为没有独立 `MotionT`/`MotionQ` 曲线,旧代码的 `RootT - MotionT` 减法退化成减 0,提取值直接原样喂给了 Hips。反编译 `mecanim::animation::EvaluateRootMotion`/`GetClipX`/`GetClipStartX`/`GetCycleX` 确认了这套"keep original"开关控制运动提取的机制。修复：`body_transform()` 按三个开关(`keep_position_xz`/`keep_position_y`/`keep_orientation`)把该轴的 RootT 分量清零(或用 swing-twist 分解掉 RootQ 的 yaw 分量)喂给 Hips 求解,同时把清零掉的那部分作为"提取的运动"单独返回；调用方(`animation_builder.py`/`HumanoidClipBaker.cs`)把这部分运动烘到骨架 OBJECT 自身的 location/rotation_quaternion(而非任何 pose bone)上,让角色真正整体前进,而不是原地摆腿。验证：对同一条 WalkLoop clip,Hips 位置误差从发散(平均 0.56,最大到 1.13)降到有界的 0.04,旋转误差从 4.4° 进一步降到 2.5°(yaw 提取后连带改善)；OBJECT 根节点的位移曲线随时间增长到约 1.13(单位同 Hips 之前发散的量级),与"角色整体前进"的预期吻合。
    - Python (`humanoid_retarget.py` 的 `_provisional_fk`/`_mass_center_of`/`_compute_mass_center`/`_compute_orientation`/`body_transform`) 与 C# (`AvatarMuscleReferential` 同名方法 + `HumanoidClipBaker.BakeHips`/`BakeSynthesizedRootMotion`) 已同步两轮修复；`animation_builder.py`/`HumanoidClipBaker.BakeHips` 的调用方也同步去掉了"`body_transform` 返回值需要再乘 hips rest 朝向"的旧逻辑——新公式返回的就是 Hips 的最终绝对 local transform，和其余骨骼走同一套"直接用，不复合 rest"的契约。

  - **★公式坑第四轮（真实事故，2026-07-10/11，续修，两个独立真 bug）**：用户要求不再肉眼看 Blender GUI，改为对一份已知正确的手工参考 `.blend`（`Anim_SaionNanae_WalkLoop_01.blend`）做 CLI 自循环逐骨骼曲线对比。结果：所有非 humanoid 骨骼（头发/眼睛/胸部等，走 `m_RotationCurves` 通用曲线路径）误差为 0，但**每一根 humanoid 肌肉驱动骨骼误差都很大**——躯干链（Spine/Chest/Neck/Head）5°~13°，四肢（UpperArm/LowerArm/UpperLeg/LowerLeg/手指）40°~85°+，且误差幅度随该关节在步态里的摆动幅度单调放大（躯干摆动小、误差小；四肢摆动大、误差大）——这个"跟摆动幅度正相关"的signature 排除了坐标系/rest 姿势之类的常数偏移，直接指向 swing 合成公式本身还有 bug，且不只一个：
    - **bug A（swing 与 twist 的合成方式本身错了）**：第三轮定论的 `swing(Y,Z)*twist(X)`（把 Y/Z 角度当一个组合轴角，再和 twist 顺序相乘）只在小角度下近似成立，角度一大就与真值分道扬镳——之前的验证之所以没测出来，是因为 187/190 样本几乎全是单轴（一次只給一个非零肌肉值）在测，从未真正验证过"两个 swing 轴同时明显非零"这种真实走路里必然出现的组合。**这次改用直接读源码而不是继续拟合候选公式**：IDA 反编译 `math::FromAxes_2`（`kZYRoll`/`m_Type=1` 分支，25 根人体骨骼统一走这条）逐条翻译 SIMD（`halfTan` 对角度向量整体求正切半角，随后一段 shuffle+乘加的叉积状运算把 X 通道和 Y/Z 通道耦合），还原出 Unity 真正的公式：`tx,ty,tz=tan(angle/2)`，`q=normalize(1, tx, ty+tx·tz, tz-tx·ty)`（w,x,y,z）——用 `HumanPoseHandler.SetHumanPose` 在真实 SaionNanae avatar 上双轴同时给值验证，新公式误差 0.000°，旧公式 5°~85°+（随角度增大而增大，与观测现象完全吻合）。
    - **bug B（四肢 twist 角度还要再乘 0.5）**：即便修完 bug A，`Left/Right {Upper,Lower}{Arm,Leg}` 的 twist 轴（DOF 0）单独测试仍有 ~18°~22° 误差，躯干/头颈链条的 twist（Spine/Chest/UpperChest/Neck/Head）单独测试误差为 0——同一个 `_muscle_angle(0.4, ...)` 在四肢上真实旋转角是预测值的**精确一半**，在躯干/头颈上精确 1:1，横跨 6+ 根骨骼、双侧都成立，不是巧合。据此在四肢四段骨骼上把 twist 角度（angle_x）额外乘 0.5——这条修正**取代**了第三轮遗留的"`Left/Right Forearm Twist In-Out` 需要对 twist 角度取反"的经验补丁（`_TWIST_SIGN_FLIP`/`TwistSignFlipMuscles`，当时在旧的 bug A 公式下诊断、根因未查明）：修完 bug A 之后重新测试 forearm 的 twist+stretch 组合姿势，"乘 0.5、不取反"直接把误差打到 <0.05°，说明旧的取反补丁是在两个 bug 叠加的症状上下的错药，现已删除并替换为 `_HALF_TWIST_BONES`/`HalfTwistBones`（Python/C# 同步）。
    - **两个 bug 都已用真实 clip 在该帧的实际肌肉值反查 Unity 验证到 6 位小数精确吻合**（例如 `LeftUpperArm` 在 walk clip 第 1 帧的真实肌肉组合 `Twist=0.233,FrontBack=0.179,DownUp=-0.777`，我们的公式与 `HumanPoseHandler` 真值逐分量相同）——这不是"看起来对"，是对真实调用点喂真实数据后跟 Unity 逐位核对。
    - **★公式坑第五轮（真实事故，2026-07-11，续修，第三个真 bug）**：用户否决"没有 IK"的猜测，明确要求继续用 IDA 读源码、禁止再猜。反编译 `math::ToAxes`（`FromAxes_2` 的精确数学逆，`atan` 还原角度、除以 limit 还原 muscle 值——两者互为逆函数，本身不引入损耗）的调用方时找到 `mecanim::human::HumanFixTwist` 与其唯一调用方 `mecanim::human::TwistSolve`：Unity 在每根骨骼各自独立求解完 muscle 之后，还会额外跑 8 对固定的 (parent, child, avatar 可配置系数) —— `(LeftLowerArm,LeftHand,m_ForeArmTwist)`、`(LeftUpperArm,LeftLowerArm,m_ArmTwist)`、右侧同构、腿部同构（`m_UpperLegTwist`/`m_LegTwist`），**顺序固定**——把 PARENT 自己的 twist 角度按系数（该 avatar 全部为 0.5，`Avatar.asset` 里 `m_ArmTwist`/`m_ForeArmTwist`/`m_UpperLegTwist`/`m_LegTwist` 实测确认）重新缩放，同时调整 CHILD 的 local 旋转以保持 CHILD **世界朝向不变**（`SkeletonAlign` 的 SIMD 没有逐位翻译，改用代数上等价、且已被本项目自己验证过的 swing-twist 公式重新推导："parent_local 变化了 delta，则 child_local_new = delta⁻¹ · child_local_old"这一步在数学上是唯一解，不依赖 Unity 内部 `SkeletonAlign` 的具体实现细节）。这解释了此前"四肢 twist 需要 ×0.5"只对上一轮验证里被当"parent 读自己"的骨骼成立，从未验证过"parent 的 twist 会让 child 的最终旋转也变"这一半——之前 `Left/Right Forearm Twist In-Out` 的取反补丁与后来的 ×0.5 补丁都只摸到了这半个机制的一角。实测 LowerArm_L 对参考文件的平均误差从 11.9° 降到 8.4°，验证了这确实是真实存在、方向正确的修复。
    - 顺手反编译了 `TwistSolve` 唯一的两个调用方之一 `mecanim::animation::EvaluateAvatarEnd`（Animator 每帧求解 avatar 姿势的收尾函数，另一个调用方是 `HumanPoseHandler::SetInternalHumanPose`，证实了本项目所有 ground-truth 测试用的 API 确实包含 TwistSolve）：函数体只有三步——拷贝当前 body skeleton pose、调用 `TwistSolve`、`MergeHumanToAvatarPose` 合并回完整 avatar 姿势——**没有第三种机制**。这是一次结构性证明（反编译了编排函数本身，不是又猜了一个假设）：本项目现在对 Unity 原生 humanoid retarget "解码方向"（muscle 曲线 → 骨骼旋转）的复刻已完整覆盖 `FromAxes_2` + `TwistSolve` 两步，没有遗漏第三步。
