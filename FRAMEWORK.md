...
- **humanoid 烘焙**（`HumanoidSolver/`）：`AvatarMuscleReferential`（Avatar Axes：PreQ/PostQ/Sgn/Limit/NormalizedRest + TOS 路径 + 全 95 肌肉 DOF 表：55 身体含眼颚 + 40 手指经 `Hand.HandBoneIndex` finger-major）+ `HumanoidClipBaker`（muscle 曲线按 SampleRate 采样 → `rest * normRest⁻¹*preQ*swingTwist*postQ⁻¹`；hips = RootT−MotionT + RootQ 前乘 rest；MotionT/Q 烘到根节点，复合恒等于完整 RootT 轨迹）。
  - **★公式坑（真实事故，2026-07-10）**：最初两轮实现都用 `postQ*swingTwist*postQ⁻¹`（对称共轭）——数值自检（L/R 镜像对称、无 NaN、帧间平滑）全部通过，视觉上也"像"在走路，但**用真实 Unity Editor（unityMCP `execute_code` + `AnimationMode.SampleAnimationClip` 在活的 Avatar+clip 上采样真实骨骼旋转）核对后发现整条腿关节方向全反**——SaionNanae 测试角色膝盖肉眼可见反向弯曲。真相：`preQ` 和 `postQ` 在真实 rig 上根本不是同一个东西（同一骨骼二者可差 60°~184°），公式必须是**非对称**的 `preQ * swingTwist * postQ⁻¹`；而且 muscle=0 时 `preQ@postQ⁻¹` 并不收敛到单位阵，而是收敛到 avatar 自己的"归一化骨架" rest 基线（`SkeletonPose.X[node].q`，即 `rest_check()` 对比的 `norm_rest`）——这个基线对某些骨骼（如 UpperLeg）离单位阵能到 ~184°，不减掉就会让该骨骼解算炸到 ~180° 偏差，同时不影响别的骨骼（如 LowerLeg 的基线恰好接近单位阵），这正是"改好一根崩掉另一根"的直接原因。最终公式 = `normRest⁻¹ * preQ * swingTwist * postQ⁻¹`，逐骨骼、多帧对比 Unity 真值验证通过。**教训：数值自洽（对称性/连续性/无 NaN）不能证明 humanoid 解算正确——这类"系统性但内部一致"的符号/基准错误必须对着 Unity 自己算出的骨骼旋转真值核对，没有捷径。**

Python (`RuriRipperImporter/humanoid_retarget.py`) 与 C# (`AvatarMuscleReferential.cs`) 两份实现已同步这个公式；此前 FRAMEWORK 记录的"1:1 移植自已验证的 humanoid_retarget.py"说法本身也是本次事故的一部分——那份"已验证"从未真正对过 Unity 真值，已在本条更新为准。
